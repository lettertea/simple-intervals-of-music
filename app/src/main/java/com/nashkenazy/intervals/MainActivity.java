...
public class MainActivity extends AppCompatActivity implements OnIntervalClick {
	...
	final private int INTERVAL_DELAY = 700;
	...
	private void setupRound() {
		...
		nextIntervalBtn.setOnClickListener(v -> {
			...
			List<Integer> includedIntervalsValues = new ArrayList<>();
			for (String interval : sharedPref.getStringSet(SettingsActivity.KEY_PREF_INCLUDED_INTERVALS,null)) {
				includedIntervalsValues.add(Integer.parseInt(interval));
			}
			Collections.sort(includedIntervalsValues);

			List<String> includedIntervalsNames = new ArrayList<>();
			for (int intervalValue : includedIntervalsValues) {
				includedIntervalsNames.add(INTERVALS.get(intervalValue));
			}

			// Set the answer according to the settings
			lowerNote = 12 * (octave - 1) + lowerNoteLetter;
			intervalDistance = includedIntervalsValues.get(RAND.nextInt(includedIntervalsValues.size()));
			...
			recyclerView.setAdapter(new MyAdapter(includedIntervalsNames, includedIntervalsValues, this));
